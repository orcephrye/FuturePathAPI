...
authentication_tasks = [
    {
        "id": 1,
        "name": "Authenticate",
        "description": "With a POST command submitting Username and Password via JSON to get a 24 hour temporary "
        'TOKEN in return. The login formate is: "{"username": <username>, "password": <password>}"',
        "uri": f"{END_POINT}/login",
    },
    {
        "id": 2,
        "name": "test",
        "description": 'Test your token works correctly. A valid response to should "{"Auth": "Auth Test"}"',
...
def authentication():
    ...
    try:
        ...
        return jsonify(
            {"Exception": "There was a failure of some kind the exception is: %s" % e}
        ), 500
    if token:
        return jsonify({"Token": "%s" % token}), 200
    else:
        return jsonify(
            {"Username": "The username %s and/or password are not correct" % username}
        ), 401
