...
def roll_from_json():
    ...
    if "rollID" in dJSON:
        return jsonify(
            {
                "rollID": dJSON.get("rollID"),
                **DieRoller.roll_dice(
                    dice=dice[0], connectors=dice[1], diceOptions=dice[2]
                ),
            }
        )

    return jsonify(
        DieRoller.roll_dice(dice=dice[0], connectors=dice[1], diceOptions=dice[2])
    )
...
class RollProbabilityGenerator(object):
    ...
    def _generateProbabilityMap(self):
        ...
        self.probabilityMap = list(
            map(caculateProbability, sorted(self.numberDict.items()))
        )
...
def _determine_numbers(dString, rerollDie=None, subAll=0, addAll=0, **kwargs):
    ...
    die = DiePicker.get_die(
        int(die.group().strip("d")), rerollDie=rerollDie, subAll=subAll, addAll=addAll
    )
    if die is None:
...
class Roller(object):
    ...
    def _drop_lowest(die, multipler, dropLowest, luck=False, explosions=0):
        ...
        if multipler <= dropLowest:
            raise Exception(
                "The number of dice to drop is greater then or equal to the number of requested "
                "dice to roll"
            )
...
class DieRoller(Roller):
    ...
    def roll_dice(dice, connectors, diceOptions):
        ...
        try:
            ...
            raise Exception("Failed to get/parse the dice options") from None
        ...
        rollsAndTotals = [
            DieRoller.get_total_from_roll(rolls, connectors) for rolls in diceRolls
        ]
    ...
    def _checkConnectors(connectors):
        if type(connectors) is not list:
            raise Exception(
                "Connectors are formatted incorrectly they are not in a list"
            )
        for connection in connectors:
            ...
            if not (connection == "-" or connection == "+"):
                raise Exception(
                    "Connectors should ebe a single character of either '+' or a '-'."
                )
        return connectors
...
class DieAnalyzer(object):
    ...
    def die_str_analyzer(dString, dieOptions=None):
        ...
        if "d" not in dString:
            raise Exception(
                "Cannot analyze string for dice. Missing 'd'. String given: %s"
                % dString
            )
    ...
    def die_json_analyzer(dJSON):
        ...
        try:
            ...
            diceOptions = tuple(
                parse_dice_options(dJSON.get("diceOptions", {})).items()
            )
            ...
            while index <= loopN:
                dies[index] = (
                    _determine_numbers(dies[index][0], **dict(dies[index][2])),
                    dies[index][1],
                    dies[index][2],
                )
                index += 1
...
class Rolling(object):
    ...
    def LowFantasyCharacterStats():
        roll = {
            "dice": [{"id": 1, "dString": "3d6", "dieOptions": {"rerollTotal": 7}}],
            "diceOptions": {"repeatRoll": 6},
        }
        return Rolling.json_roller(roll)
